...
Path('dist/gallery.js').write_text('const works = '+json.dumps(items)+';\nconst gapAfter = new Set([4,8,11,13,15,18,20,22,23,25,27,30,33,34,36,38,41,45,46,47,50]);\nconst gallery=document.querySelector("#gallery");\nfor(const item of works){const img=document.createElement("img");img.dataset.src=`assets/work/${item.number}.webp`;img.width=item.width;img.height=item.height;img.alt=`서동범 디자인 포트폴리오 — 원본 Slice ${item.number}`;img.className="reveal lazy-work";img.loading="lazy";img.decoding="async";img.addEventListener("load",()=>img.classList.add("is-loaded"),{once:true});gallery.append(img);if(gapAfter.has(item.number)){const gap=document.createElement("div");gap.className="work-gap";gap.setAttribute("aria-hidden","true");gallery.append(gap);}}\n', encoding='utf-8')
