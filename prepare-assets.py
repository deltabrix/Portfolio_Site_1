...
Path('dist/gallery.js').write_text('const works = '+json.dumps(items)+';\nconst gallery=document.querySelector("#gallery");\nfor(const item of works){const img=document.createElement("img");img.src=`assets/work/${item.number}.webp`;img.width=item.width;img.height=item.height;img.alt=`서동범 디자인 포트폴리오 — 원본 Slice ${item.number}`;img.loading="lazy";img.decoding="async";gallery.append(img);}\n', encoding='utf-8')
